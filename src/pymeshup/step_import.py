...
class STEP:
    def __init__(self, filename: Path | str, scale: float = 1.0):
        """
    ...
    def to_volume(
        ...
        copied_objects = []
        for obj in self._workplane.vals():
            # Create a true deep copy using OCCT's copy builder
            copy_builder = BRepBuilderAPI_Copy(obj.wrapped, True, True)  # copyGeom=True, copyMesh=True
            copied_obj = cq.Shape.cast(copy_builder.Shape())
            copied_objects.append(copied_obj)

        workplane_copy = cq.Workplane("XY").newObject(copied_objects)
